...
Ok, it doesn't read amazingly, but this is an API, so encoding that stuff is trivial for the client. For those of you that doesn't speak URI-coded string that is the same as:
    ...
    active = true && create_at >= 2012-01-01

Wherewolf will take that string and covert it in to ARel, so your clients can run arbitary queries against your API.

== Where we are at

Early days. The parser is done. I just need to convert that in to an ARel object...
